    ...
    render(sub) {
      ...
      <section class="panel stack" id="anchor-dg">${ui.title(T('위험물 지정수량 배수 판정', 'Designated-quantity multiple'), `${T('위험물안전관리법 제5조⑤ · 시행령 별표1', 'Dangerous Substances Act Art. 5(5) · Decree Annex 1')} ${dEx ? ui.ex() : ''}`)}
        ...
        <div class="result">
          ...
          ${ui.bars(r.rows.filter((x) => x.it && x.x != null).map((x) => ({ label: S.esc(L(x.it)), v: x.x })).concat([{ label: T('합계', 'Total'), v: r.sum, tone: r.sum >= 1 ? 'bad' : 'ok' }]),
            { unit: T('배', '×'), caption: T('품명별 배수와 합계', 'Multiples by category and total'), refs: [{ v: 1, label: T('지정수량 1배', '1× designated quantity') }].concat(r.sum * 3 >= ty[2] ? [{ v: ty[2], label: T(`예방규정 ${ty[2]}배 (${ty[1]})`, `Prevention rules ${ty[2]}× (${ty[1]})`) }] : []) })}
      ...
      <section class="panel stack" id="anchor-tq">${ui.title(T('공정안전보고서 제출 대상 판정 — 규정량', 'PSM report requirement — threshold quantities'), `${T('산안법 시행령 제43조 · 별표13', 'OSH Decree Art. 43 · Annex 13')} ${tqEx ? ui.ex() : ''}`)}
        ...
        <div class="result"><div class="row" style="justify-content:space-between"><b>R = ΣC/T</b><span class="num" style="font-size:calc(20px * var(--fz));font-weight:600">${S.fmt(tqR, 3)}</span></div>
          ${tqAny ? (tqR >= 1 ? ui.pill('bad', T('R ≥ 1 — 유해·위험설비: 공정안전보고서 작성·제출 대상 (법 제44조)', 'R ≥ 1 — hazardous installation: a PSM report is required (Act Art. 44)')) : ui.pill('ok', T('R < 1 — 규정량 미만 (증설·물질 추가 때 다시 계산)', 'R < 1 — below threshold (recalculate when capacity or substances change)'))) : `<span class="small muted">${T('수량을 입력하세요', 'Enter quantities')}</span>`}
