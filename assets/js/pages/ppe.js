      ...
      <section class="grid g2">
        ...
        <div class="result stack" aria-live="polite">
          ...
          ${ev.hr != null ? `<div class="big">${S.fmt(ev.hr, 2)} <span class="small muted">${T('유해비 = 농도 ÷', 'hazard ratio = conc ÷')} ${ev.limK}</span></div>
            ...
            ${ev.hr > 0 ? ui.dots([{ label: `<b>${T('필요 보호계수 = 유해비', 'Needed factor = hazard ratio')}</b>`, v: ev.hr, tone: 'hi', k: 0 }].concat(ev.rows.map((r) => ({ label: esc(L(r.t)), v: r.apf, tone: r.ok ? 'ok' : null, k: 1 }))).sort((a, b) => a.v - b.v || a.k - b.k),
              { caption: T('할당보호계수(APF) 순서 — 빨간 점(필요 보호계수)보다 오른쪽이어야 하고, 초록 점은 산소·정화통 등 다른 조건까지 맞아 쓸 수 있는 보호구', 'APFs in order — a respirator must sit right of the red dot (needed factor); green dots also meet the other conditions (oxygen, canister) and can be used'), aria: T('유해비와 보호구별 할당보호계수 비교', 'Hazard ratio versus each respirator’s APF') }) : ''}
