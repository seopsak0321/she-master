...
(function () {
  ...
  S.guide = {
    open(r) {
      if (r !== PROFILE && !S.GUIDES[r]) return;
  ...
  function start() {
    ...
    return `
      <section class="panel stack">${ui.title(T('처음 오셨다면 — 6곳 둘러보기', 'First time here? A six-stop tour'), T('포트폴리오를 빠르게 보는 순서', 'A quick route through the portfolio'))}
  ...
  function terms() {
    ...
    return `<section class="panel stack">${ui.title(T('용어 사전', 'Glossary'), T(`${list.length}개 · 법령·기준에서 온 정의는 출처 번호를 달았습니다`, `${list.length} terms · definitions from law or standards carry a source number`))}
      <dl class="glossary">${list.map((t) => `<div class="term" id="term-${t.id}"><dt>${S.esc(L(t.t))}</dt><dd>${L(t.d)}${S.cite(t.src || [])}${t.link ? ` <a class="xs" href="#${t.link}">${T('관련 화면', 'Related page')} →</a>` : ''}</dd></div>`).join('')}</dl>
    </section>`;
  }
  function one(r) {
    return `<section class="panel stack guide-one">
      <div class="row" style="justify-content:space-between;align-items:flex-start"><div class="stack" style="gap:2px"><span class="eyebrow">${T('가이드', 'Guide')}</span><h2 style="font-size:22px">${S.esc(rn(r))}</h2></div>
  ...
  S.pages.guide = {
    render(sub) {
      ...
      const link = (id, label) => `<a href="${id === 'start' ? '#guide' : '#guide/' + id}" ${cur === id ? 'aria-current="page"' : ''}>${S.esc(label)}</a>`;
      return `
      ${ui.head(T('도움말', 'Help'), T('이용 가이드', 'User guide'),
        T('각 기능이 무엇이고 어떻게 쓰며 현장에서 어떻게 활용할 수 있는지 설명합니다. 각 페이지 제목 옆 ‘이 페이지 가이드’ 버튼을 누르면 같은 내용을 작은 창으로 볼 수 있습니다.', 'What each feature is, how to use it and how it helps in the field. The “Page guide” button next to each page title shows the same content in a side panel.'))}
